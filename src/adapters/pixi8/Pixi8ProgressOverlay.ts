...
// Pixi8 Text constructor differs from v7: new (PIXI as any).Text({ text, style })
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function makeText(str: string, style: Record<string, unknown>): any {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export class Pixi8ProgressOverlay implements IProgressOverlay {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private _trackIdxTexts: any[] = []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private _trackNameTexts: any[] = []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private _trackTimeTexts: any[] = []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private _dcText: any
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private _dcTitleText: any
  private _stageW: number
  private _stageH: number
  private _currentTrackCount = 0
  private _currentTracks: TrackDisplayState[] = []
  private _seekLockedTrackRowIndex = -1
  ...
  constructor(stage: PIXI.Container, w: number, h: number) {
    ...
    this._dcTitleText = makeText('DC', DC_TITLE_STYLE)
    this._dcTitleText.alpha = 0.9
    this._container.addChild(this._dcTitleText)

    this._dcText = makeText('', BASE_STYLE)
  ...
  update(params: ProgressOverlayParams): void {
    ...
    // Pixi8 Graphics: clear() resets state, then use chained rect().fill() / stroke()
    this._gfx.clear()
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      // Bar background — Pixi8 API: rect().fill()
      this._gfx.rect(barX, barTop, barW, barH).fill({ color: 0xffffff, alpha: 0.1 })
      ...
      for (const m of markers) {
        ...
        this._gfx
          .moveTo(mx, barTop - 3)
          .lineTo(mx, barTop + barH + 3)
          .stroke({ width: 1.5, color: 0xfacc15, alpha: 0.85 })
      }
      ...
      nameText.text = track.animationName.length > nameMaxChars
        ? track.animationName.slice(0, nameMaxChars - 1) + '…'
        : track.animationName
    ...
    if (hasDC && tracks.length > 0) {
      ...
      if (spark.hasData && spark.linePoints.length >= 2) {
        ...
        polyPoints.push(graphX, graphTop + DC_GRAPH_H)
  ...
  private _syncTextPool(count: number): void {
    while (this._trackIdxTexts.length < count) {
      const idx  = makeText('', IDX_STYLE)
      const name = makeText('', NAME_STYLE)
